...
@interface ZYLAlert() <UIAlertViewDelegate,UIActionSheetDelegate>
{
...
- (instancetype)init
{
    if (self = [super init]) {
        ...
        _arraySheetTitle = @[@"取消"].mutableCopy;
        
        _arraySheetAction = @[@""].mutableCopy;
    }
    return self;
}
...
- (void)showActionSheetController
{
    ...
    _arraySheetTitle = [[_arraySheetTitle reverseObjectEnumerator] allObjects].mutableCopy;
    _arraySheetAction = [[_arraySheetAction reverseObjectEnumerator] allObjects].mutableCopy;
    ...
    for (int i = 0; i < _arraySheetTitle.count; i++) {
        ...
        if (i == _arraySheetTitle.count - 1) {
            ...
            UIAlertAction *action = [UIAlertAction actionWithTitle:_arraySheetTitle[i] style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
                ...
                id ac = self->_arraySheetAction[i];
                
                if (![ac isKindOfClass:[NSString class]]) {
